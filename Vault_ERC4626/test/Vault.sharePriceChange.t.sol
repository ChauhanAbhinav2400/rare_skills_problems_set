...
contract VaultSharePriceChangeTest is Test {
    ...
    function test_singleUser_protocolProfit_userGainsMore() public {
        ...
        // Alice should get 1500 tokens (her original 1000 + 500 profit)
        assertEq(mockToken.balanceOf(alice), 9000e18 + 1500e18, "Alice should get her deposit + profit");
        
        // Alice's profit = amount received - original deposit
        uint256 aliceReceived = mockToken.balanceOf(alice) - 9000e18; // What she got back
        uint256 aliceProfit = aliceReceived - initialDeposit; // Profit = received - deposited
        assertEq(aliceProfit, 500e18, "Alice should have made 500 tokens profit");
    ...
    function test_sharePrice_decreasesNewSharesWhenProfitExists() public {
        ...
        // Bob should get fewer shares: 1000 * 1000 / 1500 = 666.67 shares (rounded down)
        uint256 expectedShares = 666666666666666666666; // 666.67e18 rounded down
        assertEq(expectedBobShares, expectedShares, "Bob should get fewer shares due to higher share price");
        
        vm.startPrank(bob);
        mockToken.approve(address(vault), bobDeposit);
        vault.deposit(bobDeposit, expectedBobShares);
        vm.stopPrank();
        
        assertEq(vault.balanceOf(bob), expectedShares, "Bob should receive calculated shares");
    ...
    function test_multipleUsers_earlierDepositorBenefitsMore() public {
        ...
        vm.startPrank(bob);
        mockToken.approve(address(vault), bobDeposit);
        vault.deposit(bobDeposit, bobExpectedShares);
        vm.stopPrank();
        ...
        // Alice withdraws
        vm.startPrank(alice);
        vault.withdraw(aliceShares, aliceExpectedAssets);
        vm.stopPrank();
        
        // Bob withdraws
        vm.startPrank(bob);
        vault.withdraw(bobShares, bobExpectedAssets);
        vm.stopPrank();
        ...
        assertApproxEqAbs(
            ...
            "Total profits distributed should equal total protocol profits"
        );
    }
    ...
    function test_massiveProfit_sharePriceIncrease() public {
        ...
        vm.startPrank(bob);
        mockToken.approve(address(vault), deposit);
        vault.deposit(deposit, bobExpectedShares);
        vm.stopPrank();
        
        assertTrue(vault.balanceOf(bob) < 10e18, "Bob should get much fewer shares due to high share price");
        assertTrue(vault.balanceOf(alice) > vault.balanceOf(bob) * 10, "Alice should have more than 10x Bob's shares");
        ...
        assertTrue(aliceProfit > 900e18, "Alice should get most of the 1000e18 profit");
